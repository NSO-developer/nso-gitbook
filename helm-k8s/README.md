...
--------------------------------
  ...
  Bellow you can find a saple configuration to create a new deployment using helm-k8s NED:

  ```
  admin@ncs(config-config)# helm deployment test1
  admin@ncs(config-deployment-test1)#    chart-name    ned-charts/app2
  admin@ncs(config-deployment-test1)#    chart-version 0.1.0
  admin@ncs(config-deployment-test1)#    namespace     ned-apps
  admin@ncs(config-deployment-test1)#    app_version   0.0.1
  admin@ncs(config-deployment-test1)#    values key-value autoscaling.maxReplicas
  admin@ncs(config-key-value-autoscaling.maxReplicas)#     value 10
  admin@ncs(config-key-value-autoscaling.maxReplicas)#    exit
  admin@ncs(config-deployment-test1)# exit
  admin@ncs(config-config)#
  ```

  The new deployment named test1 will inherit the default values from the ned-charts/app2 chart and it will override the value of autoscaling.maxReplicas to 10
