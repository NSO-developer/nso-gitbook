...
------------
  ...
  This document describes the adtran-aos_nc NED.
...
------------------------------
  ...
  It is assumed the NED package `ncs-<NSO version>-adtran-aos_nc-<NED version>.signed.bin` has already
  ...
  1. Extract the NED package and verify its signature:
      ...
      > chmod u+x ncs-6.0-adtran-aos_nc-1.0.1.signed.bin
      > ./ncs-6.0-adtran-aos_nc-1.0.1.signed.bin
      ```
  ...
  2. In case the signature can not be verified (for instance if no internet connection),
     do as below instead:
      ...
      ```
      > ./ncs-6.0-adtran-aos_nc-1.0.1.signed.bin --skip-verification
      ```
  ...
  3. The result of the extraction shall be a tar.gz file with the same name as the .bin file:
      ...
      ```
      > ls *.tar.gz
      ncs-6.0-adtran-aos_nc-1.0.1.tar.gz
      ```
...
-----------------------
  ...
  1. Untar the tar.gz file. This creates a new sub-directory named:
     `adtran-aos_nc-<NED major digit>.<NED minor digit>`:

     ```
     > tar xfz ncs-6.0-adtran-aos_nc-1.0.1.tar.gz
     > ls -d */
     adtran-aos_nc-gen-1.0
     ```

  2. Install the NED into NSO, using the ncs-setup tool:

     ```
     > ncs-setup --package adtran-aos_nc-gen-1.0 --dest $NSO_RUNDIR
     ```
  ...
  ```
    > ncs-setup --package ncs-6.0-adtran-aos_nc-1.0.1.tar.gz --dest $NSO_RUNDIR
    > ncs_cli -C -u admin
    admin@ncs# packages reload
    reload-result {
      package adtran-aos_nc-gen-1.0
  ...
  ```
  > export NED_ROOT_DIR=$NSO_RUNDIR/packages/adtran-aos_nc-gen-1.0
  ```
...
------------------------
  ...
  2. Start a NSO CLI session and fetch the NED package:
     ...
     admin@ncs# software packages fetch package-from-file \
               /tmp/ned-package-store/ncs-6.0-adtran-aos_nc-1.0.tar.gz
     admin@ncs# software packages list
     package {
      name ncs-6.0-adtran-aos_nc-1.0.tar.gz
      installable
     }
     ```
  ...
  3. Install the NED package (add the argument replace-existing if a previous version has been loaded):
     ...
     ```
     admin@ncs# software packages install adtran-aos_nc-1.0
     admin@ncs# software packages list
     package {
      name ncs-6.0-adtran-aos_nc-1.0.tar.gz
      installed
  ...
  4. Load the NED package
     ...
     admin@ncs# software packages list
     package {
       name ncs-6.0-adtran-aos_nc-gen-1.0
       loaded
     }
     ```
...
-------------------------------
  ...
  - Configure a new device instance (example: dev-1):
    ...
    admin@ncs(config)# devices device dev-1 device-type generic ned-id adtran-aos_nc-gen-1.0
...
-----------------------------------
  ...
  `$NSO_RUNDIR/logs/ned-adtran-aos_nc-gen-1.0-<device name>.trace`
  ...
  4. Configure the log level for printouts to the trace file:
     ...
     ```
     admin@ncs(config)# devices device dev-1 ned-settings adtran-aos_nc logger \
     ...
     ```
     admin@ncs(config)# devices device global-settings ned-settings adtran-aos_nc logger \
                       level [debug | verbose | info | error]
     admin@ncs(config)# commit
     ```
  ...
  **IMPORTANT**:
  ...
  3. Enable Java logging with level all from the NED package:
     ...
     ```
     admin@ncs(config)# java-vm java-logging logger com.tailf.packages.ned.aos \
                       level level-all
     admin@ncs(config)# commit
     ```
  ...
  4. Configure the NED to log to the Java logger
     ...
     ```
     admin@ncs(config)# devices device dev-1 ned-settings adtran-aos_nc logger java true
     admin@ncs(config)# commit
     ```
     ...
     ```
     admin@ncs(config)# devices global-settings ned-settings adtran-aos_nc logger java true
     admin@ncs(config)# commit
     ```
...
--------------------------------------------------
  ...
  **Issues like bugs and errors shall always be reported to the Cisco NSO NED team through
  ...
  1. Enable full debug logging in the NED
     ...
     ncs_cli -C -u admin
     admin@ncs# configure
     admin@ncs(config)# devices device dev-1 ned-settings adtran-aos_nc logging level debug
     admin@ncs(config)# commit
     ```
...
------------------------
  ...
  > devices device dev-1 ned-settings adtran-aos_nc logger level debug
  > commit
  ```
...
--------------------------------------
  ...
  section 'ned-settings adtran-aos_nc live-status cli' in
