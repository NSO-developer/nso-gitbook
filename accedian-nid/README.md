...
------------------------------
    ...
    The following live-status command is available: ```mep-status {mep-name}```. 
    This show merges the output of 3 commands: ```cfm show mep configuration {mep-name}```, ```cfm show mep statistics {mep-name}``` and ```cfm show mep status {mep-name}```.
    All the fields present of the device are modeled and displayed.
    If no parameter is specified then the NED will feth and display data for all available MEP entries.
