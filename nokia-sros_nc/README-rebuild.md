...
The nokia-sros_nc NED is delivered without any YANG models in the package.
...
This list of models will be the base used by the tool when downloading the models. During operation the tool will also scan each downloaded YANG file for additional dependencies through import/include found. The tool will try to download all such dependency YANG files as well.
  ...
  When using the NETCONF protocol the YANG models can normally be downloaded directly from the device. This applies to Nokia SROS devices as well, provided the models have been properly downloaded onto the device.

  Make sure the device is configured to advertise the correct models: nokia-combined-modules and/or openconfig-modules

  Example using the Nokia SROS MD-CLI:
  ```
  [ex:/configure system management-interface yang-modules]
  A:vrnetlab@vSIM# info detail
   ## apply-groups
...
### Simple Usage
  ...
  ```
  sros-combined-from-device:          Downloads the Nokia SROS "combined" YANG models from the device,
                                      including dependencies. This is the default profile.
  sros-combined-with-bof-from-device: Downloads the Nokia SROS "combined" YANG models from the device,
                                      including the bof models. Managing bof settings through NSO does
                                      require additional NED settings configured and does have limitations.
                                      See README-ned-settings.md and the Limitations section in README.md
                                      for further info.
  openconfig-from-device:             Downloads the OpenConfig YANG models from the device, including
                                      dependencies and Nokia specific deviation/augment files.
                                      See the section *Limitations* in the README.md for further info
                                      about the openconfig support.
  ```

  Do as follows in the NSO CLI to download the files using the 'sros-combined-from-device' profile.
...
Do as follows to build each flavour of the nokia-sros_nc NED. Do it in iterations, one at the time:
   ...
   4. Follow the instructions in chapter 1.4 to reload the NED package. The rebuilt NED package
      will now have the NED-ID: `nokia-sros_nc<NED_ID_SUFFIX>-gen-<NED_ID_MAJOR>.<NED_ID_MINOR>`
...
It copies all relevant elements from the "source" directory of rebuilt NED into a tar.gz archive file. The top dir of the archive will be named in accordance with the generated NED-ID, i.e. nokia-sros_nc<NED_ID_SUFFIX>-gen-<NED_ID_MAJOR>.<NED_ID_MINOR>.
...
Below is a description of the YANG recipes currently in use by the nokia-sros_nc NED.
...
```
Path: /#protocols-config/#identifier/type
Fix: Replaced with type string
```
```
Path: /#instance-interfaces-config/leaf-list/type
Fix: Replaced with type string
```

### openconfig-bgp-common-multiprotocol.yang
```
Path: /#bgp-common-mp-afi-safi-config/#afi-safi-name/type
Fix: Replaced with type string
```

### openconfig-interfaces.yang
```
Path: /#interface-phys-config/#type/type
Fix: Replaced with type string
```
...
The process of doing scope filtering will be shown using an example. Devices running Nokia SROS support about a large number of YANG models of the flavours native and openconfig.
...
The snippet below shows the use case in XML format that will be used throughout this example. It is a super simple config, setting up some interface config using openconfig. It is good enough to illustrate the concept of build time filtering.
...
```
admin@ncs# devices device dev-1 rpc rpc-get-modules get-modules profile openconfig-from-device
...
...
fetched and saved 140 yang module(s) to /path/to/ncs-setup-dir/packages/nokia-sros_nc/src/yang
```
...
The obvious drawback is that not much footprint reduction is achieved. However, the sync-from performance can increase significantly.
...
```
admin@ncs(config)# devices device dev-1 ned-settings nokia-sros_nc connection capabilities regex-include ?
Possible completions:
  <pattern:string>
admin@ncs(config)# devices device dev-1 ned-settings nokia-sros_nc connection capabilities regex-exclude ?
Possible completions:
  <pattern:string>
```
...
admin@ncs# config
admin@ncs(config)# devices device dev-1 ned-settings nokia-sros_nc connection capabilities regex-include ".*-interfaces-.*"
admin@ncs(config)# commit
admin@ncs(config)# abort
...
```
admin@ncs(config)# devices device dev-1 ned-settings nokia-sros_nc connection capabilities regex-include openconfig-network-instance
admin@ncs(config)# devices device dev-1 ned-settings nokia-sros_nc connection capabilities regex-include openconfig-bgp
admin@ncs(config)# commit
admin@ncs(config)# abort

!Verify settings
admin@ncs# show running-config devices device dev-1 ned-settings nokia-sros_nc connection capabilities
devices device dev-1
 ned-settings nokia-sros_nc connection capabilities regex-include openconfig-network-instance
 ned-settings nokia-sros_nc connection capabilities regex-include openconfig-bgp
...
tailf-ned-nokia-sros_nc connection-stats   2024-01-16  -        -
...
```
admin@ncs# config
admin@ncs(config)# devices device dev-1 ned-settings nokia-sros_nc connection capabilities regex-include "<a suitable pattern>"
```
...
The standard scope filtering can however not be used to remove parts of the schema that reside in a certain name space.

The Nokia SROS native models is a good example of such. The absolute majority of the SROS native schema is defined in the same name space. Hence, trying to apply a standard scope filter on the native schema will not achieve much reduction at all.

Instead it is recommended to use an alternative method called *schema trimming* to reduce the SROS schema.
...
This example presumes the NED package has already been rebuilt and reloaded with the full SROS schema.

Now assume you want to trim the following schema sub trees, since they will not be used in your use case:

```
/conf:configure/service/vpls
/conf:configure/service/vprn
```
...
```
admin@ncs# show running-config devices device dev-1 config configure service vpls
% No entries found.
admin@ncs# show running-config devices device dev-1 config configure service vprn
% No entries found.
admin@ncs#
```
...
```
$ ncs_cli -C -u admin
admin@ncs# devices device dev-1 rpc rpc-rebuild-package rebuild-package filter { trim-schema { nodes [ /conf:configure/service/vpls /conf:configure/service/vprn ] } }
result ok
admin@ncs#
```
...
module nedcom-trim-deviations {
  ...
  deviation /conf:configure/conf:service/conf:vpls {
    deviate not-supported;
  }
  deviation /conf:configure/conf:service/conf:vprn {
    deviate not-supported;
  }
  deviation /conf:configure/conf:filter/conf:ip-filter/conf:embed/conf:openflow/conf:context/conf:vpls/conf:sap {
    deviate replace {
      type string {
        length 1..45;
      }
  ...
  deviation /conf:configure/conf:filter/conf:mac-filter/conf:entry/conf:action/conf:action/conf:forward/conf:forward/conf:forward-action/conf:sdp/conf:sdp/conf:vpls {
    deviate replace {
      type string {
        length 1..64;
        pattern (.*\S.*)|() {
  ...
  deviation /conf:configure/conf:filter/conf:ip-filter/conf:entry/conf:action/conf:secondary/conf:action/conf:forward/conf:forward/conf:forward-action/conf:sap/conf:sap/conf:vpls {
    deviate replace {
      type string {
        length 1..64;
        pattern (.*\S.*)|() {
  ...
  deviation /conf:configure/conf:service/conf:epipe/conf:pbb/conf:tunnel/conf:backbone-vpls-service-name {
    deviate replace {
      type string {
        length 1..64;
        pattern (.*\S.*)|() {
  ...
  deviation /conf:configure/conf:filter/conf:ipv6-filter/conf:entry/conf:action/conf:secondary/conf:action/conf:forward/conf:forward/conf:forward-action/conf:sap/conf:sap/conf:sap-id {
    deviate replace {
      type string {
        length 1..45;
      }
