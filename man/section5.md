...
> (such as 'ncs --reload').
>
> The default path to the Unix domain socket is /var/run/nso/nso-ipc,
> the value can be changed.

/ncs-config/ncs-local-ipc/enabled (boolean) \[false\]  
> If set to 'true', IPC over Unix domain socket is enabled.
>
> Note that IPC over Unix domain socket takes precedence even if values
> under ncs-ipc-address are configured.

/ncs-config/ncs-local-ipc/path (string) \[/var/run/nso/nso-ipc\]  
