...
/ncs-config/notifications/event-streams/stream/include-component-type (boolean) \[false\]  
> When set to 'true' include the leaf component-type in
> plan-state-change notifications. In NSO-6.2 and later this parameter
> has no effect and plan-state-change notifications always include
> component-type.
...
> rootDir is a directory path on disk where the system stores the result
...
> Unique cluster identifier. All members of an HA Raft cluster must be
> configured with the same cluster-name. Nodes with different
> cluster-name will not be able to connect to eachother.
...
> This parameter is mandatory.
>
> The address uniquely identifies the NSO HA node and also binds
...
> addresses allow multiple NSO HA nodes to run on the same host.
>
...
> This parameter may be given multiple times.
>
> The address of an NSO HA node that the local NSO node should try to
> connect to when starting up to establish connectivity to the HA
> cluster.

/ncs-config/ha-raft/ssl/enabled (boolean) \[true\]  
> If set to 'true', all communication between NSO HA nodes is done over
> SSL/TLS.
>
...
> Maximum number of Raft log entries that a HA node can lag behing the
...
*list*, *container*, *action*, and *refine*.
