...
Per default, the default filename of the SSH key pair will be used; for
example *id_rsa* for RSA keys or *id_ed25519* for ED25519 keys To
...
before the upgrade is performed. If not, you will loose all the package
specific configuration at reload of the NCS packages.
...
configuration. You can read more about NCS High Availability feature in
the User Guide for NCS, and in the deployment document for tailf-hcc.
...
Per default, the default filename of the SSH key pair will be used; for
example *id_rsa* for RSA keys, or *id_ed25519* for ED25519 keys. To
override this behaviour you can use the *--ssh-key-name* switch with any
NCT command.
...
target machines. Instead, create a user on the target where you install
the SSH key, and then use *sudo* to gain root privileges on the target
machine.
