...
> not given to `ncs`, first `$PWD` is searched for a file called`ncs.conf`, if not found `$NCS_DIR/etc/ncs/ncs.conf` is chosen.
...
> variable NCS\_RELOAD\_PACKAGES can be set to 'true' to attempt a package\
> reload.
...
> is equivalent to setting NCS\_RELOAD\_PACKAGES environment variable to\
> 'force'.
...
listening on its standard port/path, set the environment variables`NCS_IPC_ADDR`/`NCS_IPC_PORT` or `NCS_IPC_PATH` accordingly. The used\
values should match those specified in either the\
/ncs-config/ncs-ipc-address or /ncs-config/ncs-local-ipc of the[ncs.conf(5)](section5.md#ncs.conf) (if both sets are provided,`NCS_IPC_PATH` takes precedence). See the section on IPC in the Admin\
Guide for details.
...
> > Print help text.
>
> The output may also be filtered by file type using the _skip\_conf_,_skip\_oper_ and _skip\_snap_ options to filter out configuration,\
...
The environment variables `NCS_IPC_PORT`, `NCS_IPC_ADDR` and`NCS_IPC_PATH` control how to connect to a running NCS daemon. These\
variables generally have no effect when starting the daemon, since the\
values are read from the configuration file[ncs.conf(5)](section5.md#ncs.conf). The exception is `NCS_IPC_PATH`, which\
...
daemon log is specified in the ConfFile as described in[ncs.conf(5)](section5.md#ncs.conf).
...
"system installation", i.e. one that was done with the`--system-install` option to the NCS installer (see[ncs-installer(1)](section1.md#ncs-installer)), and "local installation" that\
...
> Specifies the directory for installation of NCS static files, like the`--install-dir` option to the installer. In the case of "system\
...
> Indicates that deviations from the module in _DeviationFile_ should be\
> present in the fxs file.
>
> This parameter can be given multiple times.
>
> By default, the _DeviationFile_ is emitted as an fxs file. To skip\
...
> Indicates that support for the YANG _features_ should be present in\
...
> compiled for import purposes. _conformance_ is a string on the form\
...
> used if no 'tailf:info' statement is present. If the option _always_\
...
> Report YANG\_ERR\_XPATH\_REF\_BAD\_CONFIG as a warning instead of an error.\
...
> translation of specific MIB objects to YANG. See[mib\_annotations(5)](section5.md#mib_annotations).
...
> objects to YANG. See [mib\_annotations(5)](section5.md#mib_annotations).
...
> parameters _--include-dir_ or _--include-file_ to specify where the\
...
> should behave in the SNMP agent. See[mib\_annotations(5)](section5.md#mib_annotations).
...
> Let _OID_ be the top object's OID. If the first component of the OID\
...
> Generate the MIB only for a subtree of the module. The _Path_ argument\
...
> Let _Name_ be the module name. If a `tailf:snmp-mib-module-name`\
...
> Generate a YANG annotation file containing the `tailf:snmp-oid`,`tailf:snmp-mib-module-name` and `tailf:snmp-row-status-column`\
...
> YANG module. If JFile is not a directory that file is created. Us&#x65;_--print-java-filename_ to get the resulting file name.
...
under `/devices/template/config` and once under`/devices/device/live-status`.
...
output of the _--ncs-compile-xxx_ command is collected.
...
> times with different `YangFileDirectory` directories and the same`--ncs-device-dir` directory to accumulate the revision history of the\
...
> module, the filenames of the YANG modules are on the form of`MOD@REVISION.yang`. The `--ncs-compile-bundle` as well as the`--ncs-compile-module` commands will rename the source YANG files and\
...
> ".miba". See [mib\_annotations(5)](section5.md#mib_annotations) .
>
...
> have been translated to YANG, _--ncs-compile-bundle_ is invoked.
>
> Each invocation of this command will populate the _--ncs-device-dir_\
> with the compiler output. This command also handles revision\
> management for NCS imported device modules. Invoke the command several\
> times with different _MibFileDirectory_ directories and the sam&#x65;_--ncs-device-dir_ directory to accumulate the revision history of the\
> modules in several different _MibFileDirectory_ directories.

`--ncs-compile-module` \<YangFile>

> This ncsc command imports a single device YANG file into th&#x65;_--ncs-model-dir_ structure. It's an alternative t&#x6F;_--ncs-compile-bundle_, however is just special case of a one-module\
...
> rare cases for adjusting the ncsc runtime footprint. For instance,_--emulator-flags="+SDio 1"_ will force the emulator to create only\
...
The ncs\_cli program is a C frontend to the NSO CLI engine. The `ncs_cli`\
program connects to NSO and basically passes data back and forth from\
the user to NSO.

ncs\_cli can be invoked from the command line. If so, no authentication\
is done. The archetypical usage of ncs\_cli is to use it as a login shell\
...
> controlled by either this flag, or the UNIX environment variable`NCS_IPC_ADDR`. The `-A` flag takes precedence.
...
> value will be the actual cwd where ncs\_cli is invoked.
...
> only made available to application code. See "built-in variables" in[clispec(5)](section5.md#clispec) and `maapi_get_user_session_opaque()` in[confd\_lib\_maapi(3)](section3.md#confd_lib_maapi). The string can be given\
> either via this flag, or via the UNIX environment variable`NCS_CLI_OPAQUE`. The `-O` flag takes precedence.
...
> Set by openssh and used by _ncs\_cli_ to determine client IP address\
> etc.
...
or as directly on the command line using the `-c string` option. The`ncs_cmd` expects commands separated by semicolon (;) or newlines. A\
...
> This option is used to also include a full backup (as produced by[ncs-backup(1)](section1.md#ncs-backup)) of the system. This helps Cisco\
...
> for a given invocation of the installation script is`InstallDir/ncs-VSN`, allowing for coexistence of multiple installed\
...
>
> > Capability `cap_net_bind_service`. One of these files (normally`ncs.smp`) will be used as the NCS daemon. The files have execute\
...
> > command, with execute access restricted to the user given via`--run-as-user`. The program is not used by the core NCS daemon, but\
...
> confusion. This option instructs the installation script to include`ncs-setup` in the installation despite this.
...
> Hide all hidden nodes. By default, no nodes are hidden unless`ncs_load` has attached to an existing transaction, in which case the\
> hidden nodes are the same as in that transaction's session.

`-U`

> Unhide all hidden nodes. By default, no nodes are hidden unless`ncs_load` has attached to an existing transaction, in which case the\
> hidden nodes are the same as in that transaction's session.
...
> the `system` context, which implies that AAA rules will _not_ be\
> applied at all.
>
...
permissions and ownership will be determined by the user running`ncs_load`. Output format is specified using the `-F` option.
...
export restriction (from `tailf:export`) that are included. If the`system` context is used (this is the default), all namespaces are\
saved, regardless of export restriction. When saving the configuration\
in one of the CLI formats, the context used for this selection is always`cli`.
...
> Include _only_ operational data, and ancestors to operational data\
...
> the default is to delete and replace. (Corresponds to the`MAAPI_CONFIG_REPLACE` flag).
...
> the configuration has been loaded. (Corresponds to the`MAAPI_CONFIG_AUTOCOMMIT` flag).
...
> Start a transaction to load _only_ operational data, and ancestors to\
...
> \<operationalMode> in a clispec file (see[clispec(5)](section5.md#clispec))
>
> ```
...
> ```
>
> will add a `show servers` command which, when run will invoke`ncs_load -F j -p /system/servers`. This will output the configuration\
...
NCS\_MAAPI\_USID and NCS\_MAAPI\_THANDLE environment variables are\
automatically set by NCS when invoking a CLI command, but when a NETCONF\
extension RPC is invoked, only NCS\_MAAPI\_USID is set, since there is no\
...
> compiled with the ned-id _tailf-ncs-ned:lsa-netconf_.
>
...
> Specifies the package name for the lower NCS, the package is in`$NCS_DIR/packages/lsa`, or a path to the package directory containing\
...
> This optional parameter specifies the _python-class-name_ of the\
> generated `package-meta-data.xml` file. It must be in forma&#x74;_&#x6D;odule.Class_. Default value is _main.Main_.
...
> with data for the complete new network when we have run`add-to-network`, by using `ncs_load` while NCS is running, e.g. like\
> this:
>
> ```
...
> make sure we have a clean starting point for a subsequent`create-network` command.
...
> to the netsim directory where the netsim data resides. Unless the`--dir NetsimDir` option is given we will search for the netsim\
...
number of packages in $NCS\_DIR/packages/neds. So given those NED\
...
*   _NETSIM\_DIR_ if set, the value will be used instead of the`--dir Netsimdir` option to search for the netsim directory containing\
    ...
    directory structure.
* _IPC\_PORT_ if set, the ConfD instances will use the indicated number\
  and upwards for the local IPC port. Default is 5010. Use this if your\
  host occupies some of the ports from 5010 and upwards.
* _NETCONF\_SSH\_PORT_ if set, the ConfD instances will use the indicated\
...
* _NETCONF\_TCP\_PORT_ if set, the ConfD instances will use the indicated\
...
* _SNMP\_PORT_ if set, the ConfD instances will use the indicated number\
...
* _CLI\_SSH\_PORT_ if set, the ConfD instances will use the indicated\
...
calling: `ncs-project help` _\<command>_
...
After running this command, the command: _ncs-project update_ , should\
be run.
...
command _ncs-project setup_ should be used to bring the project content\
up to date.
...
A package can also be specified as _local_, which means that it exists\
...
bundle is generated using the _ncs-project export_ command.
...
> created if not existing. If not specified, the _project-name_ will be\
> used.

`-u, --ncs-bin-url` URL

> Specify the exact URL pointing to an NCS install binary. Can be &#x61;_&#x68;ttp://_ or _file:///_ URL.

`--from-bundle=<bundle_path>` URL

> Specify the exact path pointing to a bundled NCS Project. The bundle\
> should have been created using the _ncs-project export_ command.
...
After a project has been created, we need to have its`project-meta-data.xml` file updated before making use of th&#x65;_&#x6E;cs-project update_ command.
...
saves them in a tar file - a _bundle_. This exported bundle can then be\
distributed to be unpacked, either with the _ncs-project create_\
command, or simply unpacked using the standard _tar_ command.

The bundle is declared in the `project-meta-data.xml` file in th&#x65;_&#x62;undle_ section. The packages included in the bundle are leafrefs to\
...
bundle, will specify all the packages as _local_ to avoid any dangling\
...
The command uses a temporary directory called _.bundle_, the directory\
...
The tar-ball can be extracted using _tar_ and the packages can be\
...
commands will be performed to get it up to date. First a _git stash_\
will be performed in order to protect from potential data loss of any\
local changes made. Then a _git fetch_ will be made to bring in the\
latest commits from the origin (remote) git repository. Finally, the\
local branch, tag or commit hash will be restored, with a _git reset_,\
...
dependency, will have that dependency resolved. First, if &#x61;_&#x70;ackages-store_ has been defined in the `project-meta-data.xml` file.\
...
The _ncs-project update_ command is intended to be called as soon as you\
...
> Answer yes on every questions. Will cause overwriting to any earlie&#x72;_&#x73;etup.mk_ files.
...
instead use a "system install" - see the[ncs-installer(1)](section1.md#ncs-installer).
...
> are found under $NCS\_DIR/packages/neds we can just provide the name\
...
> This option generates fresh ssh keys. By default the keys in`${NCS_DIR}/etc/ncs/ssh` are used. This is useful so that the ssh keys\
...
this. Assume all the yang files for the device resides in`/path/to/yang` we need to
...
localhost (on different ports) and all running the YANG models from`/path/to/yang`
...
"system installation", i.e. one that was done with the`--system-install` option to the NCS installer (see[ncs-installer(1)](section1.md#ncs-installer)).
...
> (option `--config-dir` to the installer), run-time state files (option`--run-dir` to the installer), and log files (option `--log-dir` to\
