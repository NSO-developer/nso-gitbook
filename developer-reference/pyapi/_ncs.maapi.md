...
```python
clear_read_intent(sock, thandle) -> None
```

Clear the read intent for the transaction.
This can be used for optimization purposes by the ConfD/NCS system.
...
```python
get_read_intent(sock, thandle) -> List
```
...
```python
set_read_intent(sock, thandle, xpaths) -> None
```
...
* sock -- a python socket instance
* thandle -- transaction handle
* xpaths -- the xpaths to set read intent
...
```python
upgrade_vm(sock) -> None
```

Switch the MAAPI session to use a new schema for subsequent schema loading.
