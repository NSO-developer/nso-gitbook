...
In deployments with many devices, it can become cumbersome to handle data authorization per device. To help with this there is a rule type that works on device group membership (for more on device groups, see [Device Groups](../../operation-and-usage/cli-1/nso-device-manager.md#user\_guide.devicemanager.device\_groups)). To do this, devices are added to different device groups, and the rule type `device-group-rule` is used.
