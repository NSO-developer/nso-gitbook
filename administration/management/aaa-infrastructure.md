...
In general, authenticating access to the IPC socket is a security best practice and should always be used. NSO implements it as an access check, where every IPC client must prove that it has access to a pre-shared key. See [Restricting Access to the IPC Port](../advanced-topics/ipc-ports.md#ug.ncs_advanced.ipc.restricting) on how to enable it.
