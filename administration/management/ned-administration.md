...
Using the `/ncs:devices/device/migrate` action, you can change the NED of a device. The action migrates all configuration and service meta-data. The example [examples.ncs/device-management/ned-migration](https://github.com/NSO-developer/nso-examples/tree/6.5/device-management/ned-migration) in the NSO examples collection illustrates how to migrate devices between different NED versions using this action. The actual migration procedure consists of:
