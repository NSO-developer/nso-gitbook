...
The `ncs.conf` file is described in the NSO [Manual Pages](../../../man/section5.md#configuration-parameters). There is a large number of configuration items in `ncs.conf`, most of them have sane default values. The `ncs.conf` file is an XML file that must adhere to the `tailf-ncs-config.yang` model. If we start the NSO daemon directly, we must provide the path to the NCS configuration file as in:
