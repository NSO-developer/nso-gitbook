...
*   To check the Java (JDK) and Python versions included in the container, use the following command, (where `cisco-nso-prod:6.1` is the image you want to check):

    {% code title="Example: Check Java and Python Versions of Container" %}
    ```bash
    docker run --rm cisco-nso-prod:6.1 sh -c "java -version && python --version"
    ```
    {% endcode %}
