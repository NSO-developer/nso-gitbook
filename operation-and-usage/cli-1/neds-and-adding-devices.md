...
Network Element Drivers, NEDs, provides the connectivity between NSO and the devices. NEDs are installed as NSO packages. For information on how to add a package for a new device type, see NSO [Package Management](../../administration/management/package-management.md).
...
*   **Data-Model:** Independent of underlying device interface technology NEDs come with a data model in YANG that specifies configuration data and operational data that is supported for the device.&#x20;
    ...
    * For CLI devices the NED designer wrote the YANG to map the CLI.&#x20;
    ...
    NSO only cares about the data that is in the model for the NED. The rest is ignored. See the [NED documentation](../../development/advanced-development/developing-neds/) to learn more about what is covered for the NED.
