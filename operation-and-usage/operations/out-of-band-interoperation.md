...
commands that can also be `confirm-network-state`-enabled, such as device`sync-from` and service `re-deploy`.

However, the recommended way for normal, day-to-day use is to enable`confirm-network-state` for a set of devices through device settings. For\
example:
...
Commit and other operations then no longer require using the`confirm-network-state` option explicitly; it is enabled automatically for those\
devices.
...
scenes this mechanims uses the same transaction read-set that is also used for[concurrency checks](../../development/core-concepts/nso-concurrency-model.md).
...
configuration belongs to which service through[backpointer references](../../development/advanced-development/developing-services/services-deep-dive/#ch_svcref.refcount)\
...
removed (`at-delete`), or has it gotten a new value (`at-value-set`)? The`at-create` operation makes little sense for configuration that is provisioned\
...
all current out-of-band data for a device is processed, you can invoke a`confirm-network-state`-enabled sync-from for this device, such as:
...
10.2.2.3 to 10.2.2.5, and NSO starts processing the rule set, it selects the`manage-by-service` action for this change because:
...
out-of-band changes, you can enable and set the level for`out-of-band-policy-log` in `ncs.conf`.

### Policy Rule Filter Expression

Note that `path` in the policy rule definition is a special variant of YANG`instance-identifier` that may be absolute or relative to`/devices/device/config`. As such, it is limited to selecting data nodes, and\
predicates can only be used for selecting keys.

On the other hand, `filter-expr` can specify a full XPath 1.0 expression that\
allows fine-grained selection of which rule applies where. It also supports`filter-expr`-specific extensions to the XPath language in the form of\
...
a sub-node of the rule's `path` value, for example it could be`ip/address/primary/address` even though `path` points to `ip/address`.
...
| **RULE\_PATH** | `path` value of the rule that is evaluating.                                                         |
...
the `oob:rule-paths()` would produce a node`.../bgp[as-no='65000']/neighbor[id='192.168.1.1']`. The filter expression is\
...
re-deploy would reprovision it (unless you also[detach](out-of-band-interoperation.md#attach-and-detach-out-of-band-data) out-of-band data).\
...
NSO will reapply the relevant policies if you use the`commit confirm-network-state re-evaluate-policies` commit variant when updating\
...
If you have already committed the updated service instance without`confirm-network-state re-evaluate-policies`, or have just updated the\
...
service out-of-band data are two service `re-deploy reconcile` actions:`attach-non-service-config` and `detach-non-service-config`.
...
The out-of-band data behaves like it had a policy `sync-from-device` instead of`manage-by-service`.
...
You should use attach, instead of a regular `re-deploy reconcile`, when[importing existing services to NSO](../../development/advanced-development/developing-services/services-deep-dive/#ch_svcref.discovery).\
...
processing](out-of-band-interoperation.md#introducing-confirm-network-state) described in this section,\
configure:
...
Inspect [service-managed out-of-band changes](out-of-band-interoperation.md#service-managed-out-of-band-data)\
for a service:
