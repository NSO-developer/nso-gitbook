...
NSO also fetches the host keys if required, and tries to synchronize the configuration from the device, making it ready to operate in a single step. However, the NED must explicitly support configuration synchronization in the initial transaction, otherwise this part is skipped and you must perform a manually sync-from after the transaction.
...
See [examples.ncs/device-management/netconf-call-home](https://github.com/NSO-developer/nso-examples/tree/6.4/device-management/netconf-call-home) for an example.
