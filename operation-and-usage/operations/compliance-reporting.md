...
* `outformat`: One of the formats from `xml`, `html`, `text`, or `sqlite`. If `xml` is specified, the report will be formatted using the DocBook schema. The generated file can be [downloaded](compliance-reporting.md#d5e4911) using standard CLI tools like `curl` or `wget` via the URL returned by NSO.
