...
The juniper-junos_nc NED is delivered without any YANG models in the package.
...
Do as follows to build each flavour of the juniper-junos_nc NED. Do it in iterations, one at the time:
   ...
   4. Follow the instructions in chapter 1.4 to reload the NED package. The rebuilt NED package
      will now have the NED-ID: `juniper-junos_nc<NED_ID_SUFFIX>-gen-<NED_ID_MAJOR>.<NED_ID_MINOR>`
...
It copies all relevant elements from the "source" directory of rebuilt NED into a tar.gz archive file. The top dir of the archive will be named in accordance with the generated NED-ID, i.e. juniper-junos_nc<NED_ID_SUFFIX>-gen-<NED_ID_MAJOR>.<NED_ID_MINOR>.
...
Below is a description of the YANG recipes currently in use by the juniper-junos_nc NED.

## 6.1 General

The YANG recipes used by this NED are divided into two groups. One for JunOS EX models and one for JunOS Standard + EVO.
Most of the fixes are related to some YANG constructs where leaf-lists with type empty are being used.
Such constructs are illegal according to the YANG specifation
...
...
...
fetched and saved 140 yang module(s) to /path/to/ncs-setup-dir/packages/juniper-junos_nc/src/yang
```
...
admin@ncs# packages reload
reload-result {
    package juniper-junos_nc-gen-1.1
    result true
}
```
...
```
admin@ncs(config)# devices device dev-1 ned-settings juniper-junos_nc general capabilities regex-include ?
Possible completions:
  <pattern:string>
admin@ncs(config)# devices device dev-1 ned-settings juniper-junos_nc general capabilities regex-exclude ?
Possible completions:
  <pattern:string>
```
...
admin@ncs# config
admin@ncs(config)# devices device dev-1 ned-settings juniper-junos_nc general capabilities regex-include ".*-interfaces-.*"
admin@ncs(config)# commit
admin@ncs(config)# abort
...
```
admin@ncs(config)# devices device dev-1 ned-settings juniper-junos_nc general capabilities regex-include openconfig-network-instance
admin@ncs(config)# devices device dev-1 ned-settings juniper-junos_nc general capabilities regex-include openconfig-bgp
admin@ncs(config)# commit
admin@ncs(config)# abort

!Verify settings
admin@ncs# show running-config devices device dev-1 ned-settings juniper-junos_nc general capabilities
devices device dev-1
 ned-settings juniper-junos_nc general capabilities regex-include openconfig-network-instance
 ned-settings juniper-junos_nc general capabilities regex-include openconfig-bgp
...
tailf-ned-juniper-junos_nc-stats   2024-01-16  -        -
...
```
admin@ncs# config
admin@ncs(config)# devices device dev-1 ned-settings juniper-junos_nc general capabilities regex-include "<a suitable pattern>"
```
...
The scope filtering techniques described in the previous chapter do work by only including the YANG files with XML name spaces required to cover a certain scope. This works well for YANG bundles that are properly partitioned into many different name spaces, for example the Juniper JunOS models.
...
This example presumes the NED package has already been rebuilt and reloaded with the full JunOS schema.

Now assume you want to trim the following schema sub trees, since they will not be used in your use case:

```
/jc:configuration/jc-vlans:vlans
/jc:configuration/jc-access-profile:access-profile
/jc:configuration/jc-access:access
```
...
```
admin@ncs# show running-config devices device dev-1 config configuration access
% No entries found.
admin@ncs# show running-config devices device dev-1 config configuration access-profile
% No entries found.
admin@ncs# show running-config devices device dev-1 config configuration vlans
% No entries found.
admin@ncs#
```
...
```
$ ncs_cli -C -u admin
admin@ncs# devices device dev-1 rpc rpc-rebuild-package rebuild-package filter { trim-schema { nodes [ /jc:configuration/jc-vlans:vlans /jc:configuration/jc-access-profile:access-profile /jc:configuration/jc-access:access ] } }
result ok
admin@ncs#
```
...
module nedcom-trim-deviations {
  ...
  import junos-conf-access {
    prefix jc-access;
  }
  import junos-conf-access-profile {
  ...
  deviation /jc:configuration/jc-vlans:vlans {
    deviate not-supported;
  }
  deviation /jc:configuration/jc-access-profile:access-profile {
    deviate not-supported;
  }
  deviation /jc:configuration/jc-access:access {
    deviate not-supported;
  }
}
```
...
------------------------------------------------------------
  ...
  This section describes yet another method to reduce the since of the YANG schema. A very good knowledge of the YANG modelling language is required
...
The Juniper JunOS schemas do contain a large number of nodes used for configuring vlan-id:s, all YANG modelled as leaf-list of type string.

Many, possibly all, JunOS devices do use a special compacting approach when handling vlan-id:s in these lists. Rows of consecutive numbers are compacted into ranges instead, like in the example below:

```
vlan-id-list[1 2 3 4 10 11 12 13 14 1000] ->  vlan-id-list [1-4 10-14 1000]
```

The compression happens immediately when a list of vlan-id:s is configured on a JunOS device. Thereafter the vlan-id lists will always be shown in the compact format when the configuration is read from the device.

When configuration is deployed to JunOS devices through NSO, this JunOS specific behaviour does introduce an incompatibility. The NSO expects that the configuration can also be read back in exactly the same format as it was set. Hence, the compacted vlan-id lists will cause an out-of-sync issue in NSO.

This NED does have a built-in feature which can solve this kind of issues automatically. It consists of two parts:

1. The type used for the vlan-id lists is changed from string to uint16.
2. An inbound transform capable of expanding vlan-id ranges is activated. The ranges are expanded before the payload is relayed to NSO.

The feature can currently automatically expand vlan-id lists on the following nodes in the schemas:
...
```
 ...
 More nodes can be handled upon request. See section 8 in the *README.md* about how to create a feature request.

## 11.1 Activation

This feature is enabled by rebuilding the NED using the build profile *expand-vlan-range*.

### Rebuild using the built-in tool
Execute the rebuild-package rpc from the NSO CLI (or any other northbound NSO interface) using the profile argument.
