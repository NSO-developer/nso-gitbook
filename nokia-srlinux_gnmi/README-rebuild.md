...
This list serves as the basis for downloading the models. Additionally, the tool scans each downloaded YANG file for further dependencies specified by import or include statements and attempts to download all such dependent YANG files as well.
  ...
  The gNMI protocol does not natively support downloading YANG models from a device.

  However, newer versions of Nokia SR Linux provide an effective alternative: *gNOI file transfer*. This feature allows the NED to download YANG models directly from the device, provided that the model files are present on the system.

  You can verify the presence of these files using the NSO CLI or a gNOI client such as `gnoic`. The default location for these files is `/opt/srlinux/models`. If the files are stored elsewhere, the custom path must be configured within the NED to enable the transfer. For more information, refer to `README-ned-settings.md`.

  If gNOI file transfer is not feasible, the NED will automatically attempt a simple *SCP* transfer instead. This fallback requires the following additional NED settings to be configured:

  ```
    ned-settings nokia-srlinux_gnmi connection ssh remote-port <ssh port on device>
    ned-settings nokia-srlinux_gnmi connection ssh remote-dir <yang directory on device> (default: /opt/srlinux/models)
    ned-settings nokia-srlinux_gnmi connection ssh host-key <multiple options>
  ```

  As a final alternative, the NED can retrieve the files from the public Nokia SR Linux repository on GitHub.

### Simple Usage

  The downloader tool is pre-configured with two profiles:

  -  `srlinux-all-from-git`: This profile downloads Nokia SR Linux YANG models from the official Nokia SR Linux GitHub repository. It excludes files starting with `srl_nokia-tools` and, by default, downloads version `v23.10.1`.
  - `srlinux-all-from-device`: This profile downloads native YANG files directly from the Nokia SR Linux device. This functionality requires the device to support direct downloads, primarily via the *gNOI* Get RPC or *SCP*.

  To use the `srlinux-all-*` profiles in the NSO CLI, follow the instructions below.

  **Download directly from the device:**
  ...
  **Override the default version during download:**

  ```
  # devices device dev-1 rpc rpc-get-modules get-modules profile srlinux-all remote { git { checkout <selected version> } }
  ```

  **Specify an archive file as the download source:**

  ```
  # devices device dev-1 rpc rpc-get-modules get-modules profile srlinux-all remote { archive <path to archive file> }
  ```

  For more advanced options regarding the downloader tool, please refer to Chapter **3**.

  Additionally, Chapter **5.2** of the `README.md` file provides further details, including a full list of available command-line arguments.
