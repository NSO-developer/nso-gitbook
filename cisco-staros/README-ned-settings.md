...
-----------------------------------
  ...
  Settings used when reading from device.
    ...
    - read show-plugin <true|false> (default true)

      Read plugin configurations.


    - read transaction-id-method <enum> (default config-checksum)

      Select the method used to calculate the transaction id (used by check-sync and
      compare-config).

      config-checksum  - Calculate the transaction id from the device built-in "show configuration
                         checksum" command (default). Fast, but relies on the device returning a
                         stable checksum.

      config-hash      - Calculate the transaction id from the actual running configuration. Slower,
                         but immune to unstable device checksums. Volatile encrypted secret values
                         are masked before hashing to avoid false out-of-sync.

      The default "config-checksum" method is fast but relies on the device
      returning a stable value from the "show configuration checksum" command.
      Some StarOS versions have been observed to return an unstable checksum
      even when the running configuration has not changed, which results in
      false out-of-sync detections. Use the "config-hash" method as a
      work-around in that case.

      Note that the "config-hash" method reads and transforms the full
      running configuration on every transaction id calculation and is
      therefore slower than the default method.

      Why encrypted secrets must be masked:
      StarOS re-encrypts every encrypted secret with a new, random cipher
      value each time the configuration is read, even when the underlying
      secret has not changed. For example, two consecutive reads of the same
      unchanged password return different cipher text:

          first  "show configuration":   encrypted password <+B0_DEVICE_ENCRYPTED_PASSWORD>
          second "show configuration":   encrypted password <+B2_DEVICE_ENCRYPTED_PASSWORD>

      If the raw cipher text were hashed, the transaction id would differ on
      every read and the NED would constantly report a false out-of-sync.
      To avoid this, "config-hash" masks such volatile encrypted values before
      hashing, so both reads above become:

          encrypted password <ENCRYPTED-VALUE-MASKED-FOR-TRANS-ID>

      IMPORTANT CONSEQUENCE:
      Because the encrypted secret values are masked out of the hash, a change
      made directly on the device to one of these secrets (for example a
      password changed via the device CLI outside of NSO) does NOT change the
      transaction id and therefore will NOT be detected as out-of-sync by
      check-sync or compare-config.
