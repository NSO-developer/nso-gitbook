...
The harmonic-cableos_nc NED is delivered without any YANG models included in the package.
...
tailf-ned-harmonic-cableos_nc-dev-meta.yang
tailf-ned-harmonic-cableos_nc-meta.yang
tailf-ned-harmonic-cableos_nc-meta-custom.yang
tailf-ned-harmonic-cableos_nc-oper.yang
tailf-ned-harmonic-cableos_nc-stats.yang
```
...
The default NED-ID is: harmonic-cableos_nc-gen-1.0


## 5.1 Rebuild With a Custom NED-ID

Do as follows to build each flavour of the harmonic-cableos_nc NED. Do it in iterations, one at the time:
...
3. Follow the instructions in chapter **1.3** to rebuild the NED:
   ...
   **Alternative 1: Use the built-in rpc to rebuild with custom NED-ID**
   ...
   This will generate a NED-ID like: `harmonic-cableos_nc-r21.6-gen-1.'`

   ```
   admin@ncs# devices device dev-1 rpc rpc-rebuild-package rebuild-package ned-id major 21 minor 6
   ```

   This will generate a NED-ID like: `harmonic-cableos_nc-gen-21.6`
   ...
   **Alternative 2: Rebuild the NED using gnu make from a shell**
   ...
   This will generate a NED-ID like: `harmonic-cableos_nc-r21.6-gen-1.0`
...
4. Follow the instructions in chapter **1.4** to reload the NED package. The rebuilt NED package will now have the NED-ID: `harmonic-cableos_nc<NED_ID_SUFFIX>-gen-<NED_ID_MAJOR>.<NED_ID_MINOR>`
...
The export tool copies all relevant files from the "source" directory of the rebuilt NED into a `.tar.gz` archive. The top-level directory of the archive will be named according to the generated NED-ID, for example: `harmonic-cableos_nc<NED_ID_SUFFIX>-gen-<NED_ID_MAJOR>.<NED_ID_MINOR>`.
...
result ok
exported to: /tmp/ncs-6.2.2-harmonic-cableos_nc-1.0.2-customized.tar.gz
```
...
Below is a description of the YANG recipes currently used by the harmonic-cableos_nc NED.

## 6.1 General

<__WRITE A GENERAL DESCRIPTION OF THE STATE OF THE DEVICE AND THE ISSUES FOUND__>

## 6.2 Fixes listed by schema paths

<__LIST ALL FIXES DONE USING THE SCHYPP TOOL HERE. OR "Not applicable" IF THERE IS NONE__>
...
<__LIST ALL FIXES DONE USING THE YPP AND JYPP TOOLS HERE. OR "Not applicable" IF THERE IS NONE__>
...
<__LIST OTHER KINDS OF FIXES MADE TO THE YANG HERE__>
### foo.yang
Fixed bad import statement etc
