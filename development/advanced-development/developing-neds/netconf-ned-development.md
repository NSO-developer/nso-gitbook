...
Netsim support in a NED is not strictly required for NSO to manage a NETCONF device. But it significantly improves the (service) development experience and many users find it invaluable.

The NED package generated by `ncs-make-package` already contains support for creating Netsim devices out of the box. However, Netsim devices being based on the ConfD system, use a number of preconfigured YANG modules to more realistically simulate a NETCONF device. Unfortunately, ConfD requires all the modules to use unique module prefixes and a possibility of a conflict can arise, preventing the use of the default Netsim setup.

In case the device uses a model with a duplicated prefix, you can recompile the preconfigured ConfD modules to use a different, non-conflicting prefix. A typical example is when the device uses a module with the `aaa` prefix that collides with the `tailf-aaa.yang` module. You can update the NED package's `netsim/Makefile` with code, such as:
...
+tailf-aaa.fxs: $(NCS_DIR)/netsim/confd/src/confd/aaa/tailf-aaa.yang
+       $(CONFDC) $(FLAGS) \
+          -c -o $@ -p tailf-aaa $<
...
Replace `tailf-aaa` with the problematic module; the `-p tailf-aaa` part specifies the new prefix to use instead of the default conflicting one.
