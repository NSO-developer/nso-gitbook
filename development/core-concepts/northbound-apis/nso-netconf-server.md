...
* `origin`: Indicates that the server supports the origin annotation. It is not advertised by default. The support for `origin` can be enabled in `ncs.conf` (see [ncs.conf(5)](../../../man/ncs.conf.5.md) in Manual Pages ). If it is enabled, the `origin` feature is advertised.
...
1. Make sure NSO is configured to listen to TCP traffic on localhost, port 2023, and disable SSH in `ncs.conf` (see [ncs.conf(5)](../../../man/ncs.conf.5.md) in Manual Pages ). (Re)start `sshd` and NSO. Or:
...
NSO itself is configured through a configuration file called `ncs.conf`. For a description of the parameters in this file, please see the [ncs.conf(5)](../../../man/ncs.conf.5.md) in Manual Pages man page.
...
If a data provider fails to return the data it is supposed to return, NSO can take one of two actions. Either it simply closes the NETCONF transport (default), or it can reply with an inline RPC error and continue to process the next data element. This behavior can be controlled with the `/ncs-config/netconf/rpc-errors` configuration parameter (see [ncs.conf(5)](../../../man/ncs.conf.5.md) in Manual Pages).
...
It is up to the application to define which streams it supports. In NSO, this is done in `ncs.conf` (see [ncs.conf(5)](../../../man/ncs.conf.5.md) in Manual Pages). Each stream must be listed, and whether it supports replay or not. The following example enables the built-in stream `device-notifications` with replay support, and an additional, application-specific stream `debug` without replay support:
...
To provide updates for `on-change` subscriptions on `operational` datastore, data provider applications are required to implement push-on-change callbacks. For more details, see the [PUSH ON-CHANGE CALLBACKS](../../../man/confd_lib_dp.3.md#push-on-change-callbacks) in the Manual Pages section of [confd\_lib\_dp(3)](../../../man/confd_lib_dp.3.md) in Manual Pages.
