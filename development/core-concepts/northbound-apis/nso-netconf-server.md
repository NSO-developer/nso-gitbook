...
* `origin`: Indicates that the server supports the origin annotation. It is not advertised by default. The support for `origin` can be enabled in `ncs.conf` (see [ncs.conf(5)](../../../man/section5.md#ncs.conf) in Manual Pages ). If it is enabled, the `origin` feature is advertised.
...
1. Make sure NSO is configured to listen to TCP traffic on localhost, port 2023, and disable SSH in `ncs.conf` (see [ncs.conf(5)](../../../man/section5.md#ncs.conf) in Manual Pages ). (Re)start `sshd` and NSO. Or:
...
NSO itself is configured through a configuration file called `ncs.conf`. For a description of the parameters in this file, please see the [ncs.conf(5)](../../../man/section5.md#ncs.conf) in Manual Pages man page.
...
If a data provider fails to return the data it is supposed to return, NSO can take one of two actions. Either it simply closes the NETCONF transport (default), or it can reply with an inline RPC error and continue to process the next data element. This behavior can be controlled with the `/ncs-config/netconf/rpc-errors` configuration parameter (see [ncs.conf(5)](../../../man/section5.md#ncs.conf) in Manual Pages).
...
The NETCONF notification standards define how clients subscribe to event streams. Except for the reserved `NETCONF` stream, the standards do not define how notifications are assigned to streams. In NSO, subscribers must select the built-in stream that carries the notifications they need. Configured streams are available to both NETCONF and RESTCONF clients.

The following table describes the notification events that NSO produces on its built-in streams. A configured stream does not necessarily produce notifications continuously. Some notifications require additional device, service, or kicker configuration.

<table><thead><tr><th valign="top">Stream</th><th valign="top">Notification Event</th><th valign="top">Definition</th><th valign="top">Stream Conditions</th></tr></thead><tbody><tr><td valign="top"><code>NETCONF</code></td><td valign="top">NETCONF base notifications: <code>netconf-config-change</code>, <code>netconf-capability-change</code>, <code>netconf-session-start</code>, <code>netconf-session-end</code>, and <code>netconf-confirmed-commit</code></td><td valign="top"><a href="https://www.ietf.org/rfc/rfc5277.txt">RFC 5277</a>, <a href="https://www.ietf.org/rfc/rfc8639.txt">RFC 8639</a>, and <a href="https://www.ietf.org/rfc/rfc6470.txt">RFC 6470</a></td><td valign="top">The default stream is defined in RFC 5277 and reserved by RFC 8639. The stream is available without an explicit <code>ncs.conf</code> entry. Configure the stream in <code>ncs.conf</code> to enable replay. Applications may publish additional notifications on this stream.</td></tr><tr><td valign="top"><code>ncs-alarms</code></td><td valign="top">New, changed, and cleared <code>alarm-notification</code> events</td><td valign="top"><code>tailf-ncs-alarms.yang</code></td><td valign="top">Notifications excluded through <code>/alarms/control/filter-types</code> are not emitted.</td></tr><tr><td valign="top"><code>ncs-events</code></td><td valign="top"><code>ncs-commit-queue-progress-event</code>, reporting state changes for persistent commit-queue items and the affected services and devices</td><td valign="top"><code>tailf-ncs-devices.yang</code></td><td valign="top">Used for commit-queue coordination in LSA clusters.</td></tr><tr><td valign="top"><code>kicker-events</code></td><td valign="top"><code>kicker-triggered</code>, containing the kicker identifier and monitored path</td><td valign="top"><code>tailf-kicker.yang</code></td><td valign="top">Emitted when a kicker invokes the <code>send-kicker-triggered</code> action; the stream does not automatically report every kicker execution.</td></tr><tr><td valign="top"><code>device-notifications</code></td><td valign="top">Device-defined notifications forwarded from managed devices. NSO adds the originating device, NED ID, device stream, and original event time as forwarding metadata.</td><td valign="top">Device YANG modules and <code>tailf-ncs-event-forwarding.yang</code></td><td valign="top">The stream must be configured in <code>ncs.conf</code>, and device notification subscriptions must be configured under <code>/ncs:devices/device/notifications</code>.</td></tr><tr><td valign="top"><code>service-state-changes</code></td><td valign="top"><code>plan-state-change</code> and <code>service-commit-queue-event</code></td><td valign="top"><code>tailf-ncs-plan.yang</code> and <code>tailf-ncs-services.yang</code></td><td valign="top">Events are emitted only for matching <code>/services/plan-notifications</code> or <code>/services/commit-queue-notifications</code> configuration.</td></tr><tr><td valign="top"><code>service-commit-queue-event</code></td><td valign="top">When a service is committed through the commit queue, this notification acts as a reference regarding the state of the service.<br>This notification is not sent unless the system has been configured to send the notification for the service type.</td><td valign="top"><code>tailf-ncs-services.yang</code></td><td valign="top">-</td></tr></tbody></table>

For a runnable example that generates and subscribes to application-defined notifications, device-forwarded notifications, and NETCONF base notifications over NETCONF and RESTCONF, see [Generating and Subscribing to NSO Northbound NETCONF and RESTCONF Notifications](https://github.com/NSO-developer/nso-examples/tree/main/northbound-interfaces/restconf-netconf-notifications). For an example that configures the `service-state-changes` stream and generates and subscribes to the `plan-state-change` and `service-commit-queue-event` notifications, see the [`netsim-vrouter` example](https://github.com/NSO-developer/nso-examples/tree/main/nano-services/netsim-vrouter).

### Defining Notification Streams <a href="#d5e533" id="d5e533"></a>

It is up to the application to define which streams it supports. In NSO, this is done in `ncs.conf` (see [ncs.conf(5)](../../../man/section5.md#ncs.conf) in Manual Pages). Each stream must be listed, and whether it supports replay or not. The following example enables the built-in stream `device-notifications` with replay support, and an additional, application-specific stream `debug` without replay support:
...
To provide updates for `on-change` subscriptions on `operational` datastore, data provider applications are required to implement push-on-change callbacks. For more details, see the [PUSH ON-CHANGE CALLBACKS](../../../man/section3.md#push-on-change-callbacks) in the Manual Pages section of [confd\_lib\_dp(3)](../../../man/section3.md#confd_lib_dp) in Manual Pages.
