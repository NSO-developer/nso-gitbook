...
The web server can be configured through settings in `ncs.conf` . See the [Manual Pages](../../man/index.md#section-5-file-formats-and-syntax) section Configuration Parameters.
...
The web server includes CGI functionality, disabled by default. Once you enable it in `ncs.conf` (see Configuration Parameters in [Manual Pages](../../man/index.md#section-5-file-formats-and-syntax)), you can write CGI scripts, that will be called with the following NSO environment variables prefixed with NCS\_ when a user has logged in via JSON-RPC:
...
The web server includes support for uploading packages to `/package-upload` using `HTTP POST` from the local host to the NSO host, making them installable there. It is disabled by default but can be enabled in `ncs.conf`; see Configuration Parameters in [Manual Pages](../../man/index.md#section-5-file-formats-and-syntax).
