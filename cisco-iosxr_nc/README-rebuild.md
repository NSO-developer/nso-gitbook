...
```
  ...
  11. Migrating From the Legacy IOSXR NETCONF NED
```
...
tailf-ned-cisco-iosxr_nc-dev-meta.yang
...
1. **Include all required model families** when downloading YANG files and rebuilding the NED.
   This is the *quick-and-dirty* approach and is prone to the problems described earlier.
2. **Rebuild the NED by selectively including (“cherry-picking”) models** from the required families so that the NED covers your use cases while avoiding overlap.
   This process is described in Chapters **8.1** and **8.2**, and an IOSXR example is provided in the optional step **15** below.



## Step 1: Install the 3PY NED for IOSXR NETCONF

Download, extract, and install the NED package on your local NSO installation as described in Chapter **1** of the [README.md](README.md).
...
```
show packages package cisco-iosxr_nc-gen-1.1
```
...
```
show running-config devices device * device-type
```
...
```
device xrv-710
 device-type netconf ned-id cisco-iosxr-nc-7.10
!
devices device xrv-242
 device-type netconf ned-id cisco-iosxr-nc-24.2
!
..
..
```
...
**Sample NSO Configuration:**
    ...
    devices device demo
     ...
     port      10883
     authgroup default
     device-type generic ned-id cisco-iosxr_nc-gen-1.1
...
2. Fetch the SSH host keys for the dummy device.
   ...
   ```
   devices device demo ssh fetch-host-keys
   ```
   ...
   fingerprint {
     algorithm ssh-rsa
     value a8:a7:39:a2:ed:7e:b8:80:1f:94:81:70:53:59:2f:bb
   }
   ```
...
```
devices device demo rpc rpc-get-modules get-modules profile native-um
devices device demo rpc rpc-rebuild-package rebuild-package ned-id { major 7 minor 10 suffix -um }
devices device demo rpc rpc-export-package export-package suffix -um-7.10 destination /tmp/prebuilt-neds
devices device demo rpc rpc-clean-package clean-package
```
...
```
devices device demo rpc rpc-clean-package clean-package
devices device demo rpc rpc-rebuild-package rebuild-package
```
...
Follow the instructions described in chapter **1** the [README.md](README.md).
...
```
reload-result {
    package my-service-base
    result false
    info [my-service-base-commit-auto-save.xml:13 Element: 'protocol' only exists for ned-id: isco-iosxr-nc-24.2:cisco-iosxr-nc-24.2,cisco-iosxr_nc-um-gen-7.10:cisco-iosxr_nc-um-gen-7.10
}
```
...
```
show packages package cisco-iosxr_nc- <tab complete>
```
...
Possible completions:
  cisco-iosxr_nc-gen-1.1    NED package for Cisco IOS XR generic NETCONF
  cisco-iosxr_nc-gen-um-7.10   NED package for Cisco IOS XR generic NETCONF
  cisco-iosxr_nc-gen-um-24.2   NED package for Cisco IOS XR generic NETCONF
```
...
```
devices device xrv-77 migrate new-ned-id cisco-iosxr_nc-um-gen-7.7 dry-run no-networking no-templates | save overwrite /tmp/xrv-77-migrate.txt
```
...
```
devices device xrv-242 migrate new-ned-id cisco-iosxr_nc-um-gen-24.2 dry-run no-networking no-templates | save overwrite /tmp/xrv-242-migrate.txt
```
...
```
modified-path {
    path /Cisco-IOS-XR-sysadmin-asr9k-envmon-ui:environment
...
modified-path {
    ...
    backward-compatible false
    affected-service {
        id /ncs:devices/ncs:device/env:rfs-services/my-service-segment-routing:my-service-segment-routing
    }
}
```
...
```
modified-path {
  path /Cisco-IOS-XR-um-config-commit-cfg:configuration/commit/auto-save/wait-time/days
  info leaf type stack has changed
  backward-compatible false
}
```
...
```
devices device xrv-710 migrate new-ned-id cisco-iosxr_nc-um-gen-7.10 no-networking no-templates
devices device xrv-242 migrate new-ned-id cisco-iosxr_nc-um-gen-24.2 no-networking no-templates
```
...
devices device demo
 device-type generic ned-id cisco-iosxr_nc-gen-1.1
!
devices device xrv-710
 device-type netconf ned-id cisco-iosxr_nc-um-gen-7.10
!
devices device xrv-242
 device-type netconf ned-id cisco-iosxr_nc-um-gen-24.2
!
```
...
```
devices device xrv-710 compare-config
devices device xrv-242 compare-config
```
...
devices device <device-name>
  device-type generic ned-id <corresponding-3PY-NED-package>
```
...
```
config
devices device xrv-710 device-type generic ned-id cisco-iosxr_nc-um-gen-7.10
devices device xrv-242 device-type generic ned-id cisco-iosxr_nc-um-gen-24.2
```
...
```
mkdir /tmp/legacy-neds
for n in 7.10 24.2; do mv $NSO_RUN_DIR/packages/cisco-iosxr-nc-$n /tmp/legacy-neds/; done
```
...
Begin by updating the `YANGPATH` variable in each Makefile to point to the new 3PY NED packages.
You can apply the change to all Makefiles at once using:

```
find $NSO_PACKAGE_INSTALL_DIR -name Makefile -exec sed -i 's/cisco-iosxr-nc-\([a-zA-Z0-9_.-]\+\)/cisco-iosxr_nc-um-gen-\1/g' {} +
```
...
```
/ncs:devices/device/config/Cisco-IOS-XR-um-interface-cfg:interfaces/interface/ipv4/Cisco-IOS-XR-um-if-ip-address-cfg:addresses/address
```

3PY NED XPath example (YANG-defined prefix):

```
/ncs:devices/device/config/um-interface-cfg:interfaces/interface/ipv4/um-if-ip-address-cfg:addresses/address
```
...
```
root.ncs__devices.device['dev'].config.Cisco_IOS_XR_um_interface_cfg__interfaces.interface['GigabitEthernet0/0/0/3.100'].ipv4.Cisco_IOS_XR_um_if_ip_address_cfg__addresses.address
```

3PY NED Maagic path (YANG-defined prefix adapted to Python identifiers):

```
root.ncs__devices.device['dev'].config.um_interface_cfg__interfaces.interface['GigabitEthernet0/0/0/3.100'].ipv4.um_if_ip_address_cfg__addresses.address
```
...
If updating all prefixes would require excessive effort, you can rebuild the 3PY NED so that it uses **module names as prefixes**, matching the legacy behavior.
However, this option should be used cautiously, as it may introduce unexpected side effects.
...
```
devices device dev-1 rpc rpc-rebuild-package rebuild-package use-module-as-prefix
```
...
```
configure
devices device dev-1 ned-settings cisco-iosxr_nc transaction accept-module-as-prefix true
commit
```
...
reload-result {
    package my-service-base
    result false
    info [my-service-base-misc.xml:57 the ned-id: cisco-iosxr_nc-um-gen-24.1:cisco-iosxr_nc-um-gen-24.1 is not possible, only ned-id: cisco-iosxr_nc-um-gen-7.10:cisco-iosxr_nc-um-gen-7.10 is.]
}
```
...
```
config
load merge my-service-config/my-service-sample.xml
...
native {
    device {
        name xrv-710
        data <rpc xmlns="urn:ietf:params:xml:ns:netconf:base:1.0" message-id="0">
               <lock>
                 <target>
                   <candidate/>
                 </target>
...
```
mkdir /tmp/xrv-710-use-cases
```

Then do the following in the NSO CLI:

```
show running-config devices device xrv-710 config | display xml | save overwrite /tmp/xrv-710-use-cases/day0.xml
config
load merge my-service-config/my-service-sample.xml
commit dry-run outformat xml | save overwrite /tmp/xrv-710-use-cases/day1.xml
```
...
```
devices device demo rpc rpc-get-modules get-modules profile native-all
```
...
```
cp $NCS_DIR/src/ncs/yang/ietf-yang-types.yang $NSO_PACKAGE_INSTALL_DIR/cisco-iosxr_nc-gen-1.1/src/yang/
cp $NCS_DIR/src/ncs/yang/ietf-inet-types.yang $NSO_PACKAGE_INSTALL_DIR/cisco-iosxr_nc-gen-1.1/src/yang/
cp $NCS_DIR/src/ncs/yang/ietf-netconf-acm.yang yang $NSO_PACKAGE_INSTALL_DIR/cisco-iosxr_nc-gen-1.1/src/yang/
```
...
```
devices device demo rpc rpc-rebuild-package rebuild-package ned-id { major 7 minor 10 suffix -customized } filter { scope { dir /tmp/xrv-710-use-cases } }
devices device demo rpc rpc-export-package export-package suffix -customized-7.10 destination /tmp/prebuilt-neds
```
